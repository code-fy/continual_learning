...
pageRank:网页的重要程度
网页的链接数量：多少网页链接到此网页
网页的链接质量：一个被多个网页链接的网页链接到另一个网络页，那么这个网页也同样重要
公式：V(si) = (1-d) + d * Sum(In(vi)) * 1/abs(Out(vj)) * V(j)
注： d 阻尼系数， 简单描述 网页i 的pr值等于1 减去阻尼系数 加上阻尼系数 乘上 所有指向网页i 的集合的所有指出网页的个数
再 乘上 该网页的pr值
...
"""
textrank4zh ：  将词转换成index，初始化词共现矩阵后并没有用词之间的相关性
                作为概率矩阵，然后按照公式迭代100次，生成最终的稳定的马尔可夫链
